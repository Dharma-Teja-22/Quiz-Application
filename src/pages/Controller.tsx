...
export default function Controller() {
  ...
  const [playerStatus, setPlayerStatus] = useState<Player2 | null>({
    score: 0,
    rank: 0,
  });
  ...
  useEffect(() => {
    ...
    const localGameStatus = localStorage.getItem("localGameStatus") as
      | "waiting"
      | "playing"
      | "paused"
      | "finished"
    ...
    if (
      ...
      isGameStarted &&
      localGameStatus &&
      ["waiting", "playing", "paused", "finished"].includes(
  ...
  useEffect(() => {
    ...
    socket.on(
      "question",
      (question, timeLeft, type, isFinalQuestion, quizId) => {
        if (quizId === gameId) {
  ...
  return (
    <div className="h-full relative overflow-hidden bg-[url('./assets/bgImage.png')] bg-cover bg-center flex items-center justify-center">
      <Confetti
      ...
      <div className="max-w-2xl relative w-[90%] h-full overflow-hidden flex justify-center items-center">
        <div className="bg-miracle-white w-full rounded-lg border border-gray-200 px-6 pb-5">
            ...
            <div className="flex justify-between">
              <div className="flex flex-col justify-center">
                <h5 className=" text-miracle-black font-bold text-2xl">
                  {useGameStore.getState().gameStatus === "finished" ? (
                    "Leaderboard"
                  ) : (
                    <span>
                      Quiz ID <br /> <p className="text-gray-400">{gameId}</p>
                    </span>
                  )}
                </h5>
              </div>
          ...
          <hr></hr>
          {gameStarted ? (
            useGameStore.getState().gameStatus === "paused" ? (
              ...
              <>
                ...
                <div className="grid md:grid-cols-2 grid-cols-1 gap-4 pb-5">
                  {currentQuestion.options.map((option, index) => (
                    <Button
                      key={index}
                      disabled={
                        answerIndex === -1 ? false : answerIndex !== index
                      }
                      className={`p-6 relative text-left text-white transition-all ${
                        answerIndex === index && "animate-bounce"
                      } rounded-sm ring-[#00aae7]/70 ${
                        index == 0 && "bg-[#d33257]"
                      } ${index == 1 && "bg-miracle-lightBlue"} ${
                        index == 2 && "bg-[#221f1f]"
                      } ${index == 3 && "bg-[#f7623d]"}`}
                    >
                      {/* Black: #000000
                      ...
                      <div className="absolute top-0 left-0 w-full h-full flex justify-between items-center text-white">
                        ...
                        <div className="mr-2">
                          {totalPlayers !== null &&
                            totalPlayers !== 0 &&
                            Number(
                              ((option.count / totalPlayers) * 100).toFixed(2)
                            ) + "%"}
                        </div>
                      </div>
                    </Button>
                ...
                {answerIndex >= 0 && (
                  <div className=" bg-miracle-darkBlue text-white text-center w-full p-2 italic font-semibold text-2xl">
                ...
                <div>
                  {timeLeft === 0 &&
                    useGameStore.getState().gameStatus === "finished" && (
                      <button
                        onClick={handleToppers}
                        className="p-2 mt-5 bg-miracle-darkBlue text-white rounded-lg"
                      >
                        Leaderboard
                      </button>
                    )}
                          ...
                          <Table className="">
                            <TableHeader>
                              <TableRow className="bg-black ">
                                ...
                                <TableHead className="w-[70px] text-right font-bold text-white">
                                  Score
                                </TableHead>
                              </TableRow>
                            </TableHeader>
                                ...
                                sortedStudents
                                  ...
                                  .map((player: Player, index) => (
                                    <TableRow
                                      className={`${
                                        index % 2 == 0
                                          ? "text-white bg-miracle-lightBlue "
                                          : "text-white bg-miracle-darkBlue "
                                      }`}
                                    >
            ...
            <h3 className="text-xl flex items-center justify-center font-semibold text-center text-miracle-darkGrey h-24">
